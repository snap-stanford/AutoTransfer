...
import parameter_expressions
from utils import json2parameterlegacy
from nni.tuner import Tuner
from nni.utils import OptimizeMode, extract_scalar_reward, split_index, json2space
...
def perturbation(hyperparameter_type, value, resample_probablity, uv, ub, lv, lb, random_state):
    ...
    if random.random() < resample_probablity:
        if hyperparameter_type == "choice":
            return value.index(parameter_expressions.choice(value, random_state))
        else:
    ...
    for key in hyper_parameters.keys():
        ...
        if key == 'load_checkpoint_dir' or key == 'save_checkpoint_dir':
            ...
            if isinstance(choices[0], list):
                choices = choices[0][0]
            logger.info(f"choices var for type {key} is {choices}")
        ...
        if search_space[key]["_type"] == "choice":
            idx = perturbation(search_space[key]["_type"], choices,
...
class PBTTuner(Tuner):
    ...
    def update_search_space(self, search_space):
        ...
        for i in range(self.population_size):
            hyper_parameters = json2parameterlegacy(
                self.searchspace_json, self.random_state)
