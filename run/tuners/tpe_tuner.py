...
from nni.common.hpo_utils import OptimizeMode, format_search_space, format_parameters
...
def suggest_parameter(args, rng, spec, parameter_history):
    ...
    if spec.categorical:
        if isinstance(spec.values[0], list):
            prior_weight = [item * len(spec.values[0][1]) for item in
            spec.values[0][1]]
            size = len(spec.values[0][0])
        else:
            prior_weight = args.prior_weight
            size = spec.size
        return suggest_categorical(args, rng, parameter_history, size, prior_weight)
...
def suggest_categorical(args, rng, param_history, size, prior_weight):
    """
    ...
    p = (counts + prior_weight) / sum(counts + prior_weight)  # calculate weight of good choices
    ...
    p = (counts + prior_weight) / sum(counts + prior_weight)  # calculate weight of bad choices
