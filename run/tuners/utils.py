    ...
    if isinstance(x, dict):
        if NodeType.TYPE in x.keys():
            ...
            if is_rand:
                ...
                Rand = True
            if Rand is True:
                if _type == 'choice' and type(_value[0]) is not list:
...
def json2parameterlegacy(ss_spec, random_state):
    ...
    if isinstance(ss_spec, dict):
        if NodeType.TYPE in ss_spec.keys():
            ...
            if _type == 'choice' and type(_value[0]) is not list:
                _index = random_state.randint(len(_value))
                chosen_params = json2parameterlegacy(ss_spec[NodeType.VALUE][_index], random_state)
            else:
                chosen_params = getattr(parameter_expressions, _type)(*(_value + [random_state]))
        else:
            chosen_params = dict()
            for key in ss_spec.keys():
                chosen_params[key] = json2parameterlegacy(ss_spec[key], random_state)
    elif isinstance(ss_spec, list):
        chosen_params = list()
        for _, subspec in enumerate(ss_spec):
            chosen_params.append(json2parameterlegacy(subspec, random_state))
    else:
        ...
        if spec.categorical:
            if spec.type == 'randint':
                ...
                if isinstance(spec.values[0], list):
                    _assign(ret, key, spec.values[0][0][x])
                else:
