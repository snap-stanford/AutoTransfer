...
def update_out_dir_nni(out_dir, cfg):
    cfg.out_dir = os.path.join(out_dir, os.path.join('nni', get_cfg_hash(cfg)))
    # TODO: remove dict in cfg that shouldn't be count
    makedirs_rm_exist(cfg.out_dir)    
...
def get_cfg_hash(cfg):
    ...
    time_hash = list(str(int(time.time())))
    time_hash.insert(-4, '_')
    time_hash.insert(-8, '_')
    return ''.join(time_hash) + '_' + dhash.hexdigest()
