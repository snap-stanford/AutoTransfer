...
def load_pyg(name, dataset_dir):
    ...
    if not hasattr(dataset.data, 'train_mask') and cfg.dataset.task == 'node':
        transform = RandomNodeSplit(split='train_rest', num_val=cfg.dataset.split[1], num_test=1 - cfg.dataset.split[0] - cfg.dataset.split[1])     
        dataset.data = transform(dataset.data)
        
    if not hasattr(dataset.data, 'train_graph_index') and cfg.dataset.task == 'graph':
        num_graph = len(dataset)
        ids = np.random.permutation(num_graph)
        dataset.data.train_graph_index = ids[: int(cfg.dataset.split[0] * num_graph)].tolist()
        dataset.data.val_graph_index = ids[int(cfg.dataset.split[0] * num_graph) : int((cfg.dataset.split[0] + cfg.dataset.split[1])* num_graph)].tolist()
        dataset.data.test_graph_index = ids[int((cfg.dataset.split[0] + cfg.dataset.split[1])* num_graph) : ].tolist()
        
    if not hasattr(dataset.data, 'train_edge_index') and cfg.dataset.task == 'link_pred':
        transform = RandomLinkSplit(num_val=cfg.dataset.split[1], num_test=1 - cfg.dataset.split[0] - cfg.dataset.split[1])     
        tmp = transform(dataset.data)
        dataset.data.train_edge_index = tmp[0].edge_label_index
        dataset.data.train_edge_label = tmp[0].edge_label
        dataset.data.val_edge_index = tmp[1].edge_label_index
        dataset.data.val_edge_label = tmp[1].edge_label
        dataset.data.test_edge_index = tmp[2].edge_label_index
        dataset.data.test_edge_label = tmp[2].edge_label
        # TODO: current fix doesn't work with TU dataset (e.g., dataset with multiple graphs that needs slices)
    return dataset
