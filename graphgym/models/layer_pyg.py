...
class GATConv2Head(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        super(GATConv2Head, self).__init__()
        self.model = pyg.nn.GeneralConv(dim_in, dim_out, bias=bias, attention=True, heads=2)

    def forward(self, batch):
        batch.x = self.model(batch.x, batch.edge_index)
        return batch
    
class GATConv4Head(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        super(GATConv4Head, self).__init__()
        self.model = pyg.nn.GeneralConv(dim_in, dim_out, bias=bias, attention=True, heads=4)

    def forward(self, batch):
        batch.x = self.model(batch.x, batch.edge_index)
        return batch
    
class GATConv8Head(nn.Module):
    def __init__(self, dim_in, dim_out, bias=False, **kwargs):
        super(GATConv8Head, self).__init__()
        self.model = pyg.nn.GeneralConv(dim_in, dim_out, bias=bias, attention=True, heads=8)
...
layer_dict = {
    ...
    'sageconv': SAGEConv,
    'gatconv': GATConv,
    'gatconv2head': GATConv2Head,
    'gatconv4head': GATConv4Head,
    'gatconv8head': GATConv8Head,
